...
const RIDER_SOUTH_SCRIPT = "rider_passes_south"
const RIDER_HORIZONTAL_OFFSET = 88
const RIDER_NORTH_OFFSET = 30
const RIDER_SOUTH_OFFSET = 20
...
func handle_script(script_name):
	...
	elif script_name == RIDER_SOUTH_SCRIPT: rider_south_animation()

func rider_north_animation():
	spawn_offset = Vector2(RIDER_HORIZONTAL_OFFSET, -RIDER_NORTH_OFFSET)
	animated_rider = get_node_or_null("RiderAnimationNorth")
	maybe_animate_rider("rider_north")

func rider_south_animation():
	spawn_offset = Vector2(RIDER_HORIZONTAL_OFFSET, RIDER_SOUTH_OFFSET)
	animated_rider = get_node_or_null("RiderAnimationSouth")
	maybe_animate_rider("rider_south")

func maybe_animate_rider(anim_name):
	if animated_rider: animate_rider(anim_name)
